...
This branch is a historical snapshot of the 2022 instance, preserved as a reproducible, containerized setup.
...
podman run -d --name local-postgres \
    ...
    -e POSTGRES_USER=u_nostr_2022 -e POSTGRES_PASSWORD=nostr -e POSTGRES_DB=db_nostr_2022 \
...
# postgres://u_nostr_2022:nostr@postgres:5432/db_nostr_2022
```
...
The app is available at http://localhost:8022.
...
| `DATABASE_URL` | `postgres://u_nostr_2022:<password>@postgres:5432/db_nostr_2022` — use `postgres` as the host with compose (see Option A), `localhost` with a local Postgres |
