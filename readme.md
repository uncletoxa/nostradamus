...
This branch is a historical snapshot of the 2020 instance, preserved as a reproducible, containerized setup.
...
podman run -d --name local-postgres \
    ...
    -e POSTGRES_USER=u_nostr_2020 -e POSTGRES_PASSWORD=nostr -e POSTGRES_DB=db_nostr_2020 \
...
# postgres://u_nostr_2020:nostr@postgres:5432/db_nostr_2020
```
...
The app is available at http://localhost:8020.
...
| `DATABASE_URL` | `postgres://u_nostr_2020:<password>@postgres:5432/db_nostr_2020` — use `postgres` as the host with compose (see Option A), `localhost` with a local Postgres |
