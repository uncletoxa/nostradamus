...
This branch is a historical snapshot of the 2024 instance, preserved as a reproducible, containerized setup.
...
No local Python or Postgres install needed. The app's `compose.yaml` joins an external `nostr_shared` network (the same pattern used in production to share one Postgres instance across site versions — see `deployment.md` in the main checkout), so for local use create that network and a local Postgres container once:

```bash
podman network create nostr_shared
podman run -d --name local-postgres \
    --network nostr_shared --network-alias postgres \
    -e POSTGRES_USER=u_nostr_2024 -e POSTGRES_PASSWORD=nostr -e POSTGRES_DB=db_nostr_2024 \
...
# postgres://u_nostr_2024:nostr@postgres:5432/db_nostr_2024
```
...
The app is available at http://localhost:8024.
...
| `DATABASE_URL` | `postgres://u_nostr_2024:<password>@postgres:5432/db_nostr_2024` — use `postgres` as the host with compose (see Option A), `localhost` with a local Postgres |

## Production deployment

See `deployment.md` in the main checkout for the full Podman Compose production setup, multi-version subdomain hosting, Caddy configuration, and database backup/restore procedures — it documents adding a historical version like this one as a `v20YY` worktree.
